...
variable "vpc_id" {
  description = "VPC ID where the Envoy proxy will be deployed. If not provided, will use the VPC ID from proxy-vpcs/wenko-prod-proxy remote state"
  type        = string
  default     = null
}
...
variable "appcompute_state_key" {
  description = "Terraform state key path for appcompute (e.g., 'prod/terraform.tfstate')"
  type        = string
  default     = "prod/terraform.tfstate"
}

variable "appcompute_vpc_name" {
  description = "Name of the appcompute VPC (e.g., 'qs-prd-ec1-main-vpc-v1'). If not provided, will default based on environment"
  type        = string
  default     = null
}
