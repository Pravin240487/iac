...
variable "vpc_id" {
  description = "VPC ID where the VPN will be attached. If not provided, will use the VPC ID from proxy-vpcs/wenko-prod-proxy remote state"
  type        = string
  default     = null
}
...
variable "route_table_ids" {
  description = "List of route table IDs to propagate VPN routes to. If not provided, will use route table IDs from proxy-vpcs/wenko-prod-proxy remote state"
  type        = list(string)
  default     = []
}
